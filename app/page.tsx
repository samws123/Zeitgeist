        ...
        <header className="border-b-4 border-black p-4 sm:p-6 bg-white/40 backdrop-blur-md">
          <div className="flex justify-between items-center gap-4 relative">
            ...
            <div className="hidden lg:flex absolute left-1/2 -translate-x-1/2 items-center gap-2">
              <span className="text-xs sm:text-sm font-bold">Copy Trade</span>

              <button
                onClick={() => setSelectedPlatform(selectedPlatform === "polymarket" ? "kalshi" : "polymarket")}
                className="relative inline-flex items-center bg-white border-2 border-black rounded-full p-0.5 shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-shadow"
              >
                {/* Sliding background */}
                <div
                  className={`absolute top-0.5 h-[calc(100%-4px)] bg-black rounded-full transition-all duration-300 ease-in-out ${
                    selectedPlatform === "polymarket" ? "left-0.5 w-[100px]" : "left-[calc(100px+2px)] w-[70px]"
                  }`}
                />

                {/* Text labels - stay in place */}
                <span
                  className={`relative z-10 px-3 py-1 rounded-full font-black text-xs transition-colors duration-300 w-[100px] text-center ${
                    selectedPlatform === "polymarket" ? "text-white" : "text-gray-400"
                  }`}
                >
                  Polymarket
                </span>
                <span
                  className={`relative z-10 px-3 py-1 rounded-full font-black text-xs transition-colors duration-300 w-[70px] text-center ${
                    selectedPlatform === "kalshi" ? "text-white" : "text-gray-400"
                  }`}
                >
                  Kalshi
                </span>
              ...
              <span className="text-xs sm:text-sm font-bold">Whales for Free</span>
            </div>
                ...
                {selectedTrader ? (
                  ...
                  <>
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex gap-0 bg-white border-4 border-black rounded-xl p-1 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                        <button
